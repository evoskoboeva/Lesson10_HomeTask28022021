...
public class Car extends Vehicle{
    ...
    public String toString() {
        return      "Car  " +" price " + (getPrice()<0? "incorrect price":getPrice()) +" , year= "+(getYear()<0? "incorrect year":getYear())+
                " , speed= "+(getSpeed()<0? "incorrect speed": getSpeed())
        +" , carryCapacuty= " + (carryCapacuty<0? "incorrect capacity": carryCapacuty);
    }
    ...
    public Car(double price, int year, int speed, int carryCapacuty) {
        super(price, year, speed);
        this.setCarryCapacuty(carryCapacuty);
    }
}
